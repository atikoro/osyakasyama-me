# Something like podcast

[Something like blog](https://blog.osyakasyama.me/wiliki/)の記事を、AI音声による朗読形式で配信するPodcastプロジェクトです。

- Podcast: <https://atikoro.github.io/osyakasyama-me/>
- RSS: <https://atikoro.github.io/osyakasyama-me/feed.xml>
- 制作方針: [`podcast/EDITORIAL_POLICY.md`](podcast/EDITORIAL_POLICY.md)
- AI音声レビュー: [`podcast/AI_AUDIO_REVIEW.md`](podcast/AI_AUDIO_REVIEW.md)

## ライセンス

このリポジトリの内容は、特記がない限り、原ブログと同じ
[Creative Commons 表示-非営利 4.0 国際（CC BY-NC 4.0）](https://creativecommons.org/licenses/by-nc/4.0/deed.ja)
のもとで提供します。正式なライセンス条文は [`LICENSE`](LICENSE) を参照してください。

原作記事の著者は pleasure666 です。再利用する場合は、著者名、原文URL、ライセンス名、および変更を加えた場合はその旨を表示してください。商用利用は認められていません。
