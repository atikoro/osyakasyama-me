...
第3話は、2024年10月1日に公開された「オレオレコンテキストおじさん」です。
...
マックを正確に伝えるためにはコンテキスト、つまり言葉の背景を明確にしなければなりません。

さらにマクドナルドはともかくマッキントッシュを知らない人はそれ相応にいると思われますから、そこを考慮するとより詳細に説明する必要がでてきます。

この辺の話は等しいのスコープとふわふわ時間をお読みください。

ここで触れたブログエントリ「等しいのスコープ」と「ふわふわ時間」、そして後ほど触れる「コンテキストスイッチとタスク分割コスト」へのリンクを、説明欄に掲載しています。
...
だから、一つ一つの案件、つまりコンテキストを詳細には把握しきれないのでしょう。自分は偉くなったことはないので、知らんけど。
...
そして、コンテキストの認識の欠如を自覚することは、作業そのもの、つまりコンテンツを把握するよりも難易度の高い行為です。

「AをBにする」だったら、それをその通りにすればいいだけですが、「AをBにする背景、つまり経緯」となると、個人個人で微妙に認識がずれてきます。
...
仕事を進める前に、まずはみんなの中のイマジナリーコンテキストを集約して、明確に文章化した「ワンピース、つまり、ひとつなぎの大コンテキスト」を定義すべきなのです。

今回は、Something like blogの「オレオレコンテキストおじさん」を、できるだけ原文のまま朗読してお届けしました。
